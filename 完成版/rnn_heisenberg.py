...
# -*- coding: utf-8 -*-
"""
RNN-VMC: 自旋 1/2 反铁磁 Heisenberg 链 (PBC, L=10)。
==================================================
参考:
  [1] M. Hibat-Allah, M. Ganahl, L. E. Hayward, R. G. Melko, J. Carrasquilla,
      "Recurrent Neural Network Wave Functions", PRX 10, 031017 (2020) [arXiv:2002.02973]
  [2] 本方 NQS 范式 (参考 nqs_gcnn.py): 批量 ln_psi / 批量 E_loc / VMC-Adam

波函数 (cRNN, 复数相位, 不用 Marshall 符号):
    ψ(σ) = exp(iφ(σ)) · √P(σ) · 1[Σσ=0]        (Sz=0 投影, 论文零磁化配额)
    P(σ) = ∏_i p(σ_i|σ_<i)   —— GRU + Softmax (幅度)
    φ(σ) = Σ_i φ_i, φ_i = π·Softsign(U2 h_i + c2)  (相位/符号层)

能量 (取 S^z = σ/2):
    H = Σ_i [ S^z_i S^z_{i+1} + (1/2)(S^+_i S^-_{i+1} + S^-_i S^+_{i+1}) ]
    E_loc(σ) = (1/4)Σ_i σ_i σ_{i+1} + (1/2)Σ_{i:σ_i≠σ_{i+1}} ψ(σ^i)/ψ(σ)

梯度 (score-function, GCNN 范式):
    loss = cov(Re E_loc, ln|ψ|) + cov(Im E_loc, φ),   p.grad = (2/N)·∇loss
    (等于 ∂E/∂θ = 2Re⟨(E_loc−E)∂logψ*⟩, 已验证与有限差分一致)

两种训练模式:
  --mode det   确定性: 枚举 Sz=0 全构型, E = ψ†Hψ/ψ†ψ 精确 (NQS 范式, 无噪声)
  --mode mc    MC: 自回归采样 |ψ|², VMC-Adam 随机梯度 (可扩展大系统)

用法:
  python rnn_heisenberg.py --L 10 --dh 32 --steps 2000 --mode det
  python rnn_heisenberg.py --L 10 --dh 32 --steps 2000 --mode mc --ns 300
"""
import torch
import math
import random
...
class RNNHeisenberg(torch.nn.Module):
    ...
    def __init__(self, L, dh=32, seed=None, sz0=True, device='cpu',
                 ...
                 magnetization=0, twist=0.0, softsign_phase=False):
        super().__init__()
        ...
        self.softsign_phase = softsign_phase  # 有界相头 φ=π·Softsign(w_i) (论文 Eq.145)
    ...
    def ln_psi(self, states):
        ...
        for i in range(L):
            ...
            if self.softsign_phase:
                phi = phi + torch.pi * self._softsign(w_im[ar, idx[:, i]])  # φ=π·Softsign(w_i), 每 site 有界
            else:
                phi = phi + w_im[ar, idx[:, i]]       # 相位 φ=Im w (默认无界)
