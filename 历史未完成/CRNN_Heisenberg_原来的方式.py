...
class Complex_RNN:
    ...
    def Gate(self,state,h):
        ...
        u_n = torch.sign(torch.mv(self.Wu,pi1)+self.bu)
        r_n = torch.sign(torch.mv(self.Wr,pi1)+self.br)
    ...
    def E_local(self,state,ln_psi):
        ...
        for i in range(self.L):
            ...
            state2[(i + 1) % self.L] = -state2[(i + 1) % self.L]
            h_next1 = self.h0
            h_next2 = self.h0
            # 计算左右两个格点自旋翻转后构型的概率
            for j in range(self.L):
                ...
                h_next2 = self.Gate(self.sigma[int(state2[j].item())], h_next2)
                y1R = torch.softmax(torch.mv(self.U1, h_next1) + self.c1, dim=0)
                y2R = torch.softmax(torch.mv(self.U1, h_next2) + self.c1, dim=0)
                y1C = math.pi * self.Softsign(torch.mv(self.U2, h_next1) + self.c2)
                y2C = math.pi * self.Softsign(torch.mv(self.U2, h_next2) + self.c2)

                a = int((1 - (-1) ** state1[j].item()) / 2)
                b = int((1 - (-1) ** state2[j].item()) / 2)
                lnP_relate1 += torch.log(y1R[a])
                lnP_relate2 += torch.log(y2R[b])
                ln_phi1 += torch.dot(y1C, self.sigma[a])
                ln_phi2 += torch.dot(y2C, self.sigma[b])
                ln_psi1 = 1j * ln_phi1 + 0.5 * lnP_relate1
                ln_psi2 = 1j * ln_phi2 + 0.5 * lnP_relate2
                E_loc += torch.exp(ln_psi1 - ln_psi) * 0.5 + torch.exp(ln_psi2 - ln_psi) * 0.5
        return E_loc
    ...
    def CRNN(self):
        ...
        for i in range(self.L):
            ...
            if R<P1:
                ...
                ln_P += torch.log(P1)
                ln_phi += torch.dot(yn2,self.sigma1)
            else:
                state[i] = -1
                state_initial = self.sigma0
                ln_P += torch.log(P2)
                ln_phi += torch.dot(yn2,self.sigma0)

        ln_psi = 1j*ln_phi+0.5*ln_P#对于这个构型展开系数的对数值
    ...
    def P_update(self):
        ...
        for i in range(self.step):
            ...
            (ln_psi.conj()).real.backward()

            # 收集梯度
            params_vector = torch.cat([
                self.Wu.flatten(),
                self.Wr.flatten(),
                self.bu.flatten(),
                self.br.flatten(),
            ...
            partial_E += params_vector*E_loc
            E += E_loc
        E = E/self.step
        partial_E = self.alpha*partial_E.real/self.step*2
        '''参数的更新'''
        idx = 0
        '''self.Wu的更新'''
    ...
    def learning(self):
        ...
        for i in range(1000):
            E = self.P_update()
            print(f'第',i,f'步的能每个格点能量为',E/self.L)
