...
class VMC:
    def __init__(self,N):
        ...
        self.sigma = torch.stack([self.sigma1,self.sigma0])
        self.dv = 2
        self.dh = 2
    ...
    def RNN(self):
        ...
        for j in range(self.N):
            state_relate = state
            state_relate[j] = -state_relate[j]
            log_P_relate = 0
            for i in range(self.N):
                pi = torch.cat((h_next, self.sigma[int(state_relate[i].item())]), dim=0)
                h_next = torch.nn.functional.elu(torch.mv(self.W, pi) + self.b)
                y = torch.softmax(torch.mv(self.U, h_next) + self.c, dim=0)
                if state_relate[i]==1:
                    log_P_relate += torch.log(y[0])
                else:
                    log_P_relate += torch.log(y[1])
            E_loc += torch.exp(log_P_relate-log_P)*self.h*0.5
        print(E_loc)
        E = self.Ising(state)+E_loc
