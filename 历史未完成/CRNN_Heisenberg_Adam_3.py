...
class Complex_RNN:
    def __init__(self, L):
        ...
        self.dh = 4
        self.dv = 2
        self.Wu = (torch.rand((self.dh, self.dh + self.dv), device=device1, dtype=dtype1)*2-1)
        self.Wr = (torch.rand((self.dh, self.dh + self.dv), device=device1, dtype=dtype1)*2-1)
        self.bu = (torch.rand(self.dh, device=device1, dtype=dtype1)*2-1)
        self.br = (torch.rand(self.dh, device=device1, dtype=dtype1)*2-1)
        self.Wc1 = (torch.rand((self.dh, self.dh), device=device1, dtype=dtype1)*2-1)
        self.Wc2 = (torch.rand((self.dh, self.dv), device=device1, dtype=dtype1)*2-1)
        self.bc1 = (torch.rand(self.dh, device=device1, dtype=dtype1)*2-1)
        self.bc2 = (torch.rand(self.dh, device=device1, dtype=dtype1)*2-1)
        self.U1 = (torch.rand((self.dv, self.dh), device=device1, dtype=dtype1)*2-1)
        self.U2 = (torch.rand((self.dv, self.dh), device=device1, dtype=dtype1)*2-1)
        self.c1 = (torch.rand(self.dv, device=device1, dtype=dtype1)*2-1)
        self.c2 = (torch.rand(self.dv, device=device1, dtype=dtype1)*2-1)
